...
void metadata::printMetadata()
{
	...
	struct tm *ptminfo;
	ptminfo = localtime(&birthTime);
	cout << " Birth: " << ptminfo->tm_year+1900 << "-" << ptminfo->tm_mon + 1 << "-" << ptminfo->tm_mday << " ";
	...
	ptminfo = localtime(&changeTime);
	cout << "Change: " << ptminfo->tm_year+1900 << "-" << ptminfo->tm_mon + 1 << "-" << ptminfo->tm_mday << " ";
	if (ptminfo->tm_hour < 10) cout << "0" << ptminfo->tm_hour << ":";
	else cout << ptminfo->tm_hour << ":";
	if (ptminfo->tm_min < 10) cout << "0" << ptminfo->tm_min << ":";
	else cout << ptminfo->tm_min << ":";
...
string genFindPkt(unsigned int ID, int type)
{
	string sendTemp = "";
	if(type == 1)
	{
